...
export default async function setRpc({ urls }: SetRpcOptions) {
  const chainIds = await Promise.all(urls.map(getChainIdByRpcUrl));
  const newRpcUrls = chainIds.reduce(
    (acc, chainId, index) => {
  ...
  globalConfig.set('rpcUrls', {
    ...globalConfig.get('rpcUrls'),
    ...newRpcUrls,
  });

  console.log(chalk.green('RPC URLs set successfully'));
}
